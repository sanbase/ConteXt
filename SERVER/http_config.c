...
char auth_pem_encrypt[MAX_STRING_LEN]; uyguygig
...
void process_server_config(FILE *errors) 
{
	...
	strcpy(cx_path,DEFAULT_CX_PATH);
	...
	while(!(cfg_getline(l,MAX_STRING_LEN,cfg))) 
	{
		...
		if((l[0] != '#') && (l[0] != '\0')) 
		{
			...
			{
				if(!strcasecmp(l,"inetd"))
					standalone=0;
				else if(!strcasecmp(l,"standalone"))
					standalone=1;
				else 
				{
...
int parse_access_dir(FILE *f, int line, char orr, char *dir,char *file, FILE *out)
{
	...
	if(sec==NULL)
		sec=(security_data *)calloc(MAX_SECURITY,sizeof (security_data));
	...
	while(!(cfg_getline(l,MAX_STRING_LEN,f))) 
	{
		...
		else if(!strcasecmp(w,"<Limit")) 
		{
			...
			while(w2[0]) 
			{
				...
				else if(!strcasecmp(w,"RUN")) m[M_RUN]=1;
			}
			while(1) 
			{
